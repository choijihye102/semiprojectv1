...
const  validInputs = (form) =>{
 ...
 //오류메세지 출력  - 어디다 찍을건지 -      inputs[2], 무엇을 찍을건지. -errorMessages[2];
...
// fetch 방식: 비동기 처리 가능, 에러처리는 세밀하게 조절가능, 요청데이터를 JSON으로 전송 가능, REST API/SPA 에서 주로 사용

// 프라미스 ( promise )
// 자바스크립트에서 비동기 작업을 처리하기 위한 객체
// 주로 비동기 작업의 성공/실패를 관리하고 결과를 처리하는데 사용.
// ES6에서 처음 도입 -  콜백 지옥 해결, 비동기처리 코드를 깔끔하게 작성.
// async :  비동기 함수임을 선언 - 반환값은 promise임
// await :  비동기 함수의 처리가  완료될때까지 기다렸다가 결과를 받아옴. .  계란 익기전에 값 가져오지말고 전자렌지 다 돌면 땡해줄께!.
